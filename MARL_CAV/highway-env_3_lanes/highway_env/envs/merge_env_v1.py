    ...
    def _make_road(self) -> None:
        ...
        for i in range(3):
            ...
            net.add_lane(
                ...
                "d",
                StraightLane(
                    [sum(self.ends[:3]), y[i]], [sum(self.ends), y[i]], line_types=line_type[i]
                ),
            )
        ...
        amplitude = 3.25
        merging_y = 4 * StraightLane.DEFAULT_WIDTH + 2*StraightLane.DEFAULT_WIDTH /3   # Position below the 3rd lane
        ljk = StraightLane(
            [0, merging_y], [self.ends[0], merging_y], line_types=[c, c], forbidden=True
        )
        lkb = SineLane(
    ...
    def _make_vehicles(self, num_CAV=4, num_HDV=3) -> None:
        ...
        for _ in range(num_CAV // 2):
            ...
            ego_vehicle = self.action_type.vehicle_class(
                road, 
                road.network.get_lane(("a", "b", np.random.randint(3))).position(
